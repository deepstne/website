...
> Note: These commands rely on whether a player is online or not to work properly. The location of a player who is not online will not be shown using these commands if run while a specific player is not connected to the server.
...
### Homepage
The server's homepage is where everyone comes when opening `darkst.one` in their browser. You know, the website telling you that it's the network made for you. The homepage offers informational pages about the network's servers, rules and staff players. You can find these pages by using the navigation bar above.
...
### Community forum
We run a community forum for collaboration and official business at [talk.darkst.one](https://talk.darkst.one). Forum membership is not required to participate on the server, but recommended to dive in deeper with our community. However, an account is required to apply for our various [titles and honors](../hc/titles-and-honors) and to receive those benefits. The community forum is hosted by us and runs the [Discourse](https://www.discourse.org/) forum software.
